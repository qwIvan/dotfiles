...
RUN bash -c "comm -23 <(comm -12 <(sort package_list) <(pacman -Slq | sort)) <(pactree -sru xproto | sort) > /tmp/tmplist && \
      comm -23 /tmp/tmplist <(pacman -Fl - < /tmp/tmplist | grep 'usr/share/applications/.*\.desktop$' | cut -d \  -f 1 | uniq) > /tmp/has-desktop && \
